...
void setup() {
  ...
  CAN0.init_Filt(4, 0, 0x01000000);  // Init fifth filter: 1xx
  CAN0.init_Filt(5, 0, 0x01000000);  // Init sixth filter: 1xx
...
void loop() {
  ...
  if (!digitalRead(2))  // If pin 2 is low, read receive buffer
  {
    ...
    //==============[ SPEED ]==============//
    if (rxId == 0x1A1) {
      speed = ((rxBuf[3] << 8 | rxBuf[2])* 0.015625) / 1.609;       // speed is an integer
      if (speed != lastSpeed) {
        if (debug) {
          //Serial.print("Speed (HEX): ");
          //Serial.println(rxBuf[2], HEX);
          //Serial.print("Speed (RAW): ");
          //Serial.println((rxBuf[3] << 8) | (rxBuf[2]), HEX);
          Serial.print("Speed MPH: ");
          Serial.println(speed);
        }
        sprintf(speedTx, "S%02X", speed);
        //Serial.println(speedTx);
        lastSpeed = speed;
      }
    }
